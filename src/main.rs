...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        let query = prompt_input(&format_prompt("Search Song:"));
        ...
        display_songs(&songs);
        let choice = prompt_input(&format_prompt(&format!("Select (1-{}):", songs.len())))
            .trim()
            .parse::<usize>()
            .unwrap_or(0);
...
fn format_prompt(title: &str) -> String {
    let line = "=".repeat(title.len()).bright_blue();
